"""Safe-concurrency guard for the guide operator (parallel-execution migration, step 4).
...
_op = importlib.import_module("guide.operator").build_guide_operator()
...
def test_read_only_caps_declared():
    ...
    assert ro == ["guide.retrieve", "guide.walkthrough"]
