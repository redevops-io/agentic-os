...
def test_proactive_answers_are_status_honest_not_overclaiming():
    ...
    for pid in ("recruiting-fit-engine", "knowledge-debt-radar", "creator-intelligence"):
        a = proactive[pid].answer.lower()
        assert ("not shipped" in a or "roadmap" in a or "planned" in a or "not yet" in a), pid
    # the four loop producers now SHIP over the shared learning loop, validated IN SIMULATION —
    # they must say the mechanism ships AND that it is not a real-world lift claim (no over-claiming).
    for pid in ("crm-next-best-action", "outreach-intent-radar", "analytics-anomaly-action",
                "wealth-assumption-drift"):
        a = proactive[pid].answer.lower()
